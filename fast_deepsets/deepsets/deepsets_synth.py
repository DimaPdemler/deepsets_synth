...
# def deepsets_invariant_synth_custom(
#     input_size: tuple,
#     phi_layers: list = [32, 32, 32],
#     phi_activations: list = ["relu", "relu", "relu"],
#     phi_normalizations: list = [None, None, None],
...
#         return qkeras.QActivation(f"quantized_relu({nbits}, 0)", name=name)(x)
#     elif activation == "leaky_relu":
#         return qkeras.QActivation(custom_leaky_relu(nbits, leaky_relu_alpha), name=name)(x)
#     elif activation == "tanh":
#         return qkeras.QActivation(f"quantized_tanh({nbits}, 0)", name=name)(x)
#     elif activation == "sigmoid":
#         return qkeras.QActivation(f"quantized_sigmoid({nbits}, 0)", name=name)(x)
...
# def custom_leaky_relu(nbits, alpha):
#     """Custom LeakyReLU activation that doesn't require alpha to be a power of 2."""
#     def leaky_relu(x):
#         x = qkeras.quantized_relu(nbits, 0)(x)
#         neg = qkeras.quantized_relu(nbits, 0)(-x)
#         return x - alpha * neg
#     return leaky_relu
